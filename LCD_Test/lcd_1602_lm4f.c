...
void lcd_init()
{
  ...
  lcd_komut(0x28);  //  4 Bit, Dual LCD Exchange
  lcd_komut(0x0C);  // Hiding the Cursor
  lcd_komut(0x06);  // Right to correctly write active
  lcd_komut(0x80);  // The LCD is the first Exchange Location
  lcd_komut(0x28);  // 4 Bit, Dual LCD Exchange
  lcd_temizle();    // Display Flushing 
}
